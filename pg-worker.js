...
const _cloneChunkResult = chunkResult => {
  ...
  const _getHeightfieldsSize = () => {
    if (heightfields) {
      let size =
        heightfields.pixels.length *
        heightfields.pixels.constructor.BYTES_PER_ELEMENT;
      return size;
    } else {
      return 0;
    }
  ...
  const _clonePIInstances = instancesResult => {
    if (instancesResult) {
      const ps = new instancesResult.ps.constructor(
        arrayBuffer,
        index,
        instancesResult.ps.length,
      );
      ps.set(instancesResult.ps);
      index +=
        instancesResult.ps.length *
        instancesResult.ps.constructor.BYTES_PER_ELEMENT;

      const instances = new instancesResult.instances.constructor(
        arrayBuffer,
        index,
        instancesResult.instances.length,
      );
      instances.set(instancesResult.instances);
      index +=
        instancesResult.instances.length *
        instancesResult.instances.constructor.BYTES_PER_ELEMENT;
  ...
  const _cloneHeightfields = () => {
    if (heightfields) {
      const pixels = new heightfields.pixels.constructor(
        arrayBuffer,
        index,
        heightfields.pixels.length,
      );
...
const _cloneBarrierResult = barrierResult => {
  ...
  const _getLeafNodesIndexSize = () => {
    let size =
      leafNodesIndex.length * leafNodesIndex.constructor.BYTES_PER_ELEMENT;
    return size;
  };
  ...
  const _cloneLeafNodesMin = () => {
    if (leafNodes.length > 0) {
      const leafNodesMin2 = new leafNodesMin.constructor(
        arrayBuffer,
        index,
        leafNodesMin.length,
      );
  ...
  const _cloneLeafNodesMax = () => {
    const leafNodesMax2 = new leafNodesMax.constructor(
      arrayBuffer,
      index,
      leafNodesMax.length,
    );
  ...
  const _cloneLeafNodesIndex = () => {
    const leafNodesIndex2 = new leafNodesIndex.constructor(
      arrayBuffer,
      index,
      leafNodesIndex.length,
    );
...
const _handleMethod = async ({method, args, instance: instanceKey, taskId}) => {
  switch (method) {
    ...
    case 'generateBarrier': {
      const {chunkPosition, minLod, maxLod, chunkSize} = args;
      const instance = instances.get(instanceKey);
      ...
      return {
        ...
        transfers: [barrierResult2.arrayBuffer],
      };
    }
    /* case 'createMobSplat': {
